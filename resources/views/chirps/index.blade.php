...
@dump($errors->get('message'))
            ...
            <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900">
                @foreach($chirps as $chirp) {{-- Esta variable $chirps es la colección que retorna chirps.index --}}
                    <div class="p-6 flex space-x-2">
                        ...
                        {{-- @if (auth()->user()->id===$chirp->user_id) esto es lo mismo que:--}}
                        {{-- @if (auth()->user()->is($chirp->user)) en vez de esto uso @can--}}
                        @can('update', $chirp)
                         <x-dropdown>
                            <x-slot name="trigger">
                                <button><svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
